...
class TestEspnFantasyApiLoader(unittest.TestCase):
    ...
    def test__load_json(self):
        ...
        os.makedirs(os.path.join(root_folder_path, "2020"), exist_ok=True)
        os.makedirs(os.path.join(root_folder_path, "2020", "scoring_periods"), exist_ok=True)
        self._create_empty_json(os.path.join(root_folder_path, "2020", "2020_league_info.json"))
        self._create_empty_json(os.path.join(root_folder_path, "2020", "scoring_periods", "2020_scoring_period1.json"))

        # Instantiate
        espn_api = EspnFantasyApiLoader(root_folder_path)

        # Test typical cases
        self.assertIsNotNone(espn_api._load_json("2020", "2020_league_info.json"))
        self.assertIsNotNone(espn_api._load_json("2020", "scoring_periods", "2020_scoring_period1.json"))

        # Test non-existent files
        self.assertIsNone(espn_api._load_json("2020", "2019_league_info.json"))
