...
class RequestsUtil():
    ...
    def load_json_from_endpoint(self, endpoint):
        """ Loads JSON data from an endpoint into a dictionary. """
        return self._load_json(f"{self._base_url}{endpoint}")

    def save_json_from_endpoint(self, endpoint, out_file_path):
        """ Save data from the an endpoint into a JSON file output.
            Returns True if request and data saving is successful. """
        # Load data
        json_data = self._load_json(f"{self._base_url}{endpoint}")
    ...
    def load_jsons_from_endpoints_async(endpoint_list):
    ...
    def save_jsons_from_endpoints_async(self, endpoints_file_path_dict_list):
        ...
        json_data_list = asyncio.run(self._load_jsons_async(url_list))
    ...
    async def _load_jsons_async(self, url_list):
        ...
        async with aiohttp.ClientSession() as session:
            tasks = []
            for url in url_list:
                tasks.append(asyncio.create_task(self._load_json_from_session(session, url)))

            json_data_list = await asyncio.gather(*tasks)
        return json_data_list

    async def _load_json_from_session(self, session, url):
